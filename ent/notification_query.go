...
// QueryRelatedTweet chains the current query on the "related_tweet" edge.
func (nq *NotificationQuery) QueryRelatedTweet() *TweetQuery {
	query := (&TweetClient{config: nq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := nq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (nq *NotificationQuery) Clone() *NotificationQuery {
	...
	return &NotificationQuery{
		...
		withRelatedTweet: nq.withRelatedTweet.Clone(),
...
// WithRelatedTweet tells the query-builder to eager-load the nodes that are connected to
// the "related_tweet" edge. The optional arguments are used to configure the query builder of the edge.
func (nq *NotificationQuery) WithRelatedTweet(opts ...func(*TweetQuery)) *NotificationQuery {
	query := (&TweetClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	nq.withRelatedTweet = query
	return nq
}
...
func (nq *NotificationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Notification, error) {
	var (
		...
		loadedTypes = [3]bool{
			nq.withUser != nil,
			nq.withRelatedTweet != nil,
			nq.withRelatedUser != nil,
		}
	)
	if nq.withUser != nil || nq.withRelatedTweet != nil || nq.withRelatedUser != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := nq.withRelatedTweet; query != nil {
		if err := nq.loadRelatedTweet(ctx, query, nodes, nil,
			func(n *Notification, e *Tweet) { n.Edges.RelatedTweet = e }); err != nil {
			return nil, err
		}
	}
...
func (nq *NotificationQuery) loadRelatedTweet(ctx context.Context, query *TweetQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *Tweet)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Notification)
	for i := range nodes {
		if nodes[i].notification_related_tweet == nil {
			continue
		}
		fk := *nodes[i].notification_related_tweet
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "notification_related_tweet" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (nq *NotificationQuery) loadRelatedUser(ctx context.Context, query *UserQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *User)) error {
	...
	nodeids := make(map[int][]*Notification)
	for i := range nodes {
		if nodes[i].notification_related_user == nil {
			continue
		}
		fk := *nodes[i].notification_related_user
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "notification_related_user" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
