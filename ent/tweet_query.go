...
func (tq *TweetQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tweet, error) {
	...
	if query := tq.withParent; query != nil {
		if err := tq.loadParent(ctx, query, nodes, nil,
			func(n *Tweet, e *Tweet) { n.Edges.Parent = e }); err != nil {
			return nil, err
		}
	}
...
func (tq *TweetQuery) loadLikes(ctx context.Context, query *LikeQuery, nodes []*Tweet, init func(*Tweet), assign func(*Tweet, *Like)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(like.FieldTweetID)
	}
...
func (tq *TweetQuery) loadRetweets(ctx context.Context, query *RetweetQuery, nodes []*Tweet, init func(*Tweet), assign func(*Tweet, *Retweet)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(retweet.FieldTweetID)
	}
	query.Where(predicate.Retweet(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tweet.RetweetsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (tq *TweetQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := tq.ctx.Fields; len(fields) > 0 {
		...
		if tq.withParent != nil {
			_spec.Node.AddColumnOnce(tweet.FieldParentID)
		}
		if tq.withAuthor != nil {
			_spec.Node.AddColumnOnce(tweet.FieldAuthorID)
		}
	}
